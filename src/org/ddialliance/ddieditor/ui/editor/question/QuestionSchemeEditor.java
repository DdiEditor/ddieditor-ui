...
public class QuestionSchemeEditor extends Editor implements ISelectionListener,
	...
	public void createPartControl(Composite parent) {
		...
		List<TabItem> tabItemList = super
				.createStandardPartControl(
						parent,
						Messages
								.getString("QuestionSchemeEditor.label.QuestionSchemeEditorLabel.QuestionSchemeEditor"),
						Messages
		...
		final Composite questionSchemeComposite = new Composite(tabFolder,
				SWT.NONE);
		questionSchemeComposite.setBackground(Display.getCurrent()
				.getSystemColor(SWT.COLOR_WHITE));
		final GridLayout gridLayout = new GridLayout();
		questionSchemeComposite.setLayout(gridLayout);
		TabItem questionSchemeTabItem = tabItemList
				.get(TABITEM_INDEX.QUESTION_SCHEME.ordinal());
		questionSchemeTabItem.setControl(questionSchemeComposite);
		questionSchemeTabItem
				.setText(Messages
		...
		labelLabel.setBackground(Display.getCurrent().getSystemColor(
				SWT.COLOR_WHITE));
		labelLabel.setText(Messages
				.getString("QuestionSchemeEditor.label.Label")); //$NON-NLS-1$
		...
		final Label questionSchemeDescrLabel = new Label(questionGroup,
				SWT.NONE);
		final GridData gd_questionSchemeDescrLabel = new GridData(SWT.RIGHT,
				SWT.TOP, false, false);
		gd_questionSchemeDescrLabel.horizontalIndent = 5;
		questionSchemeDescrLabel.setLayoutData(gd_questionSchemeDescrLabel);
		questionSchemeDescrLabel.setBackground(Display.getCurrent()
				.getSystemColor(SWT.COLOR_WHITE));
		questionSchemeDescrLabel.setText(Messages
				.getString("QuestionSchemeEditor.label.DescriptionText.Label")); //$NON-NLS-1$

		questionSchemeDescrStyledText = new StyledText(questionGroup, SWT.WRAP
				| SWT.V_SCROLL | SWT.BORDER);
		questionSchemeDescrStyledText.addModifyListener(new ModifyListener() {
			public void modifyText(final ModifyEvent e) {
				log.debug("Description changed");
				questionScheme
						.setDescr(questionSchemeDescrStyledText.getText());
				editorStatus.setChanged();
			}
		});
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			...
			String errMess = MessageFormat
					.format(
							Messages
									.getString("QuestionSchemeEditor.mess.ValidationError"), e1.getMessage()); //$NON-NLS-1$
		...
		try {
			if (editorInput.getEditorMode().equals(EDITOR_MODE_TYPE.NEW)) {
				...
				editorInput.setEditorMode(EDITOR_MODE_TYPE.EDIT);
					...
					.equals(EDITOR_MODE_TYPE.EDIT)) {
				QuestionSchemes.update(questionScheme);
			} else if (editorInput.getEditorMode()
					.equals(EDITOR_MODE_TYPE.VIEW)) {
			...
			String errMess = MessageFormat
					.format(
							Messages
									.getString("QuestionSchemeEditor.mess.ErrorDuringSave"), e.getMessage()); //$NON-NLS-1$
			MessageDialog.openError(site.getShell(), Messages
					.getString("ErrorTitle"), errMess);
			return;
		}
		editorStatus.clearChanged();
		log
				.debug("QuestionSchemeEditor.doSave(1): "
	...
	public void init(IEditorSite site, IEditorInput input)
		...
		if (log.isDebugEnabled()) {
			log.debug("QuestionSchemeEditor.init() - Name: "
					+ editorInput.getName());
			log.debug("QuestionSchemeEditor.init() - ID: "
					+ editorInput.getId());
			log.debug("QuestionSchemeEditor.init() - Parent ID: "
					+ editorInput.getParentId());
			log.debug("QuestionSchemeEditor.init() - Editor Mode: "
		...
		if (editorInput.getEditorMode().equals(EDITOR_MODE_TYPE.NEW)) {
			try {
				questionScheme = QuestionSchemes.createQuestionScheme(
						editorInput.getId(), editorInput.getVersion(),
						editorInput.getParentId(), editorInput
								.getParentVersion());
			} catch (Exception e) {
			...
			try {
				questionScheme = QuestionSchemes.getQuestionScheme(editorInput
			...
			String errMess = MessageFormat
					.format(
							Messages
									.getString("QuestionSchemeEditor.mess.UnknownEditorMode"), editorInput.getEditorMode()); //$NON-NLS-1$
			MessageDialog.openError(site.getShell(), Messages
					.getString("ErrorTitle"), errMess);
			System.exit(0);
		}
