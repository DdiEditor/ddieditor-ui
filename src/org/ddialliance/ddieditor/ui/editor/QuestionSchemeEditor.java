...
public class QuestionSchemeEditor extends Editor implements ISelectionListener {
	...
	public void createPartControl(Composite parent) {
		...
		questionSchemeTabItem.setText(Messages.getString("QuestionSchemeEditor.label.questionSchemeTabItem.QuestionScheme")); //$NON-NLS-1$
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			...
			String errMess = MessageFormat.format(Messages.getString("QuestionSchemeEditor.mess.ValidationError"), e1.getMessage()); //$NON-NLS-1$
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		// Initialise the Stander Editor Part:
		...
		if (editorInput.getEditorMode().equals(EDITOR_MODE_TYPE.NEW)) {
			try {
				questionScheme = QuestionSchemes.createQuestionScheme(editorInput.getId(), editorInput.getVersion(), editorInput.getParentId(),
			...
			try {
				questionScheme = QuestionSchemes.getQuestionScheme(editorInput.getId(), editorInput.getVersion(), editorInput
						.getParentId(), editorInput.getParentVersion());
			} catch (Exception e) {
				String errMess = MessageFormat.format(
						Messages.getString("QuestionSchemeEditor.mess.GetQuestionSchemeByIdError"), e.getMessage()); //$NON-NLS-1$
				MessageDialog.openError(site.getShell(), Messages.getString("ErrorTitle"), errMess);
				System.exit(0);
			}
		} else {
