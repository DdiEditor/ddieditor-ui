...
public class FilteredItemsSelection {
	 ...
	 * @param preIdValue
	 */
	public void createPartControl(final Composite parentLabelComposite, final Composite parentCodeComposite, 
			String subTitleText, String itemLabelText,
		...
		if (subTitleText.length() > 0) {
			...
			final GridData gd_subTitleLabel = new GridData(SWT.RIGHT, SWT.CENTER, false, false);
			// gd_subTitleLabel.widthHint = 500;
			SubTitleLabel.setLayoutData(gd_subTitleLabel);
			SubTitleLabel.setText(subTitleText + ":");
			SubTitleLabel.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WHITE));
		}

		final Label itemLabel = new Label(labelComposite, SWT.NONE);
		itemLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, true, true));
		itemLabel.setText(itemLabelText + ":");
		itemLabel.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WHITE));
		...
		final GridData gd_composite = new GridData(SWT.FILL, SWT.CENTER, true, true, 2, 1);
		...
		comboViewer.setContentProvider(new org.eclipse.jface.viewers.ArrayContentProvider());
		...
		browseButton.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WHITE));
		browseButton.setText(Messages.getString("ResponseTypeDetail.label.Browse")); //$NON-NLS-1$
	}
	
	/**
	 * Add selection listener
	 * 
	 * @param Title of FilteredItemsSelectionDialog
	 * @param referenceList
	 * @param selectionListener
	 */
	public void addSelectionListener(final String title, final List<LightXmlObjectType> referenceList,
			SelectionListener selectionListener) {

		comboViewer.getCombo().addModifyListener(new ModifyListener() {
			public void modifyText(final ModifyEvent e) {
				result = referenceList.get(comboViewer.getCombo().getSelectionIndex());
			}
		});
		...
		browseButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				FilteredItemsSelectionDialog dialog = new FilteredItemsSelectionPopUp(shell, title, referenceList,
						false, false);
