...
public class LoopEditor extends Editor implements IAutoChangePerspective {
	...
	public void createPartControl(Composite parent) {
		...
		ReferenceSelectionCombo variableRefSelectCombo = createRefSelection(
				...
				Messages.getString("LoopEditor.label.variableref"), modelImpl
						.getLoopVariableReference(), variableRefList, false);
		...
		ReferenceSelectionCombo controlConstructRefSelectCombo = createRefSelection(
				group, Messages.getString("ControlConstruct.ref"), Messages
						.getString("ControlConstruct.ref"), modelImpl
		...
		Group initValueGroup = createSubGroup(group, Messages
				.getString("LoopEditor.label.InitValue"));

		ProgrammingLanguageCodeType initValue = modelImpl.getCode(modelImpl
				.getInitialValue());
		Text initValueTxt = createTextInput(initValueGroup, Messages
				.getString("LoopEditor.label.InitValue"),
		...
		Text appLangInitValueTxt = createTextInput(initValueGroup, Messages
				.getString("LoopEditor.label.InitValue.programlang"),
		...
		ReferenceSelectionCombo initValueQuestRefSelectCombo = createRefSelection(
				initValueGroup, Messages
						.getString("IfThenElse.editor.ifquestionref"), Messages
		...
		// loop while
		Group loopWhileGroup = createSubGroup(group, Messages
				.getString("LoopEditor.label.loopwhile"));
		ProgrammingLanguageCodeType loopWhile = modelImpl.getCode(modelImpl
				.getLoopWhile());
		Text loopWhileTxt = createTextInput(loopWhileGroup, Messages
				.getString("LoopEditor.label.loopwhile"),
		...
		Text appLangLoopWhileTxt = createTextInput(loopWhileGroup, Messages
				.getString("LoopEditor.label.loopwhile.programlang"),
		...
		// step value
		Group stepValueGroup = createSubGroup(group, Messages
				.getString("LoopEditor.label.stepvalue"));
		ProgrammingLanguageCodeType stepValue = modelImpl.getCode(modelImpl
				.getStepValue());
		Text steepValueTxt = createTextInput(stepValueGroup, Messages
				.getString("LoopEditor.label.stepvalue"),
		...
		Text appLangSteepValueTxt = createTextInput(stepValueGroup, Messages
				.getString("LoopEditor.label.stepvalue.programlang"),
		...
		try {
			Text txt = createLabelInput(group2, Messages.getString("editor.label.label"), modelImpl.getDocument()
					.getLoop().getLabelList(), modelImpl.getDocument().getLoop().getId());

			createTranslation(group2, Messages.getString("editor.button.translate"), modelImpl.getDocument()
					.getLoop().getLabelList(), new LabelTdI(), "", txt);

			StyledText styledText = createStructuredStringInput(group2, Messages.getString("editor.label.description"),
					modelImpl.getDocument().getLoop().getDescriptionList(), modelImpl.getDocument()
							.getLoop().getId());
			createTranslation(group2, Messages.getString("editor.button.translate"), modelImpl.getDocument()
					.getLoop().getDescriptionList(), new DescriptionTdI(), "", styledText);
		} catch (DDIFtpException e) {
			DialogUtil
