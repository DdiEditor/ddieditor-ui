...
public class QuestionItemEditor extends Editor implements ISelectionListener {
	...
	public class QITTableContentProvider implements IStructuredContentProvider {
		 ...
		 * 
		 * - i.e. Question Text with "true" tranlated attribute.
		 * 
		 * (non-Javadoc)
		 * 
		 * @see
	...
	private ComboViewer conceptComboViewer;
	private Combo langCombo;
	private ComboViewer responseComboViewer;
	private Composite ResponseTypeComposite;
	...
	public void createPartControl(Composite parent) {
		...
		// Concept:
		final Label conceptLabel = new Label(questionGroup, SWT.NONE);
		final GridData gd_conceptLabel = new GridData(SWT.RIGHT, SWT.CENTER, false, false);
		gd_conceptLabel.horizontalIndent = 5;
		conceptLabel.setLayoutData(gd_conceptLabel);
		conceptLabel.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WHITE));
		conceptLabel.setText(Messages.getString("QuestionItemEditor.label.conceptLabel.Concept")); //$NON-NLS-1$

		conceptComboViewer = new ComboViewer(questionGroup);
		conceptComboViewer.setContentProvider(new org.eclipse.jface.viewers.ArrayContentProvider());
		conceptComboViewer.setLabelProvider(new org.eclipse.jface.viewers.LabelProvider());
		final List<ConceptReference> conceptRerenceList = new ArrayList<ConceptReference>();

		final GridData gd_combo_1 = new GridData(SWT.FILL, SWT.CENTER, true, false);
		gd_combo_1.widthHint = 226;
		conceptComboViewer.getCombo().setLayoutData(gd_combo_1);

		// - get Concept Reference
		String conceptRef = "";
		if (!editorInput.getEditorMode().equals(EDITOR_MODE_TYPE.NEW)) {
			try {
				conceptRef = questionItem.getConceptRef();
			} catch (Exception e1) {
				String errMess = MessageFormat.format(
						Messages.getString("QuestionItemEditor.mess.ConceptReferenceRetrievalError"), e1.getMessage()); //$NON-NLS-1$
				MessageDialog.openInformation(site.getShell(), Messages.getString("InfoTitle"), errMess);
			}
		}

		// - get available Concepts
		conceptRerenceList.add(new ConceptReference("", "")); // - add Undefined
		int i = 0, index = 0;
		i++;
		try {
			for (LightXmlObjectType lightXmlObject : Concepts.getConceptsLight("", "", "", "")) {
				if (lightXmlObject.getId().equals(conceptRef)) {
					index = i;
				}
				Concept concept = Concepts.getConcept(lightXmlObject.getId(), lightXmlObject.getVersion(),
						lightXmlObject.getParentId(), lightXmlObject.getParentVersion());
				conceptRerenceList.add(new ConceptReference(concept.getFirstLabel(), lightXmlObject.getId()));
				i++;
			}
		} catch (Exception e1) {
			String errMess = MessageFormat.format(
					Messages.getString("QuestionItemEditor.mess.AvailableConceptsRetrievalError"), e1.getMessage()); //$NON-NLS-1$
			MessageDialog.openError(site.getShell(), Messages.getString("ErrorTitle"), errMess);
		}
		conceptComboViewer.setInput(conceptRerenceList);
		conceptComboViewer.getCombo().select(index);
		conceptComboViewer.getCombo().addModifyListener(new ModifyListener() {
			public void modifyText(final ModifyEvent e) {
				// Note: Store the Id - not the label - as Concept Reference
				try {
					questionItem.setConceptRef(((ConceptReference) conceptRerenceList.get(conceptComboViewer.getCombo()
							.getSelectionIndex())).getId());
				} catch (Exception e1) {
					String errMess = MessageFormat.format(
							Messages.getString("QuestionItemEditor.mess.ConceptReferenceUpdateError"), e1.getMessage()); //$NON-NLS-1$
					MessageDialog.openError(site.getShell(), Messages.getString("ErrorTitle"), errMess);
					return;
				}
		...
		ResponseTypeComposite = new Composite(questionGroup, SWT.NONE);
		ResponseTypeComposite.setRedraw(true);
		ResponseTypeComposite.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WHITE));
		final GridData gd_composite_3 = new GridData(SWT.FILL, SWT.CENTER, false, false, 2, 1);
		ResponseTypeComposite.setLayoutData(gd_composite_3);
		final GridLayout gridLayout_5 = new GridLayout();
		gridLayout_5.numColumns = 2;
		ResponseTypeComposite.setLayout(gridLayout_5);

		final ResponseTypeDetail responseTypeDetail = new ResponseTypeDetail(questionItem, ResponseTypeComposite,
				editorStatus);
			...
			public void modifyText(final ModifyEvent e) {
				...
				if (index >= 0) {
					RESPONSE_TYPES rt = ((ResponseTypeReference) responseDomainReferenceList.get(index))
							.getResponseDomain();
					// Save Respondse Type without details - saved by
