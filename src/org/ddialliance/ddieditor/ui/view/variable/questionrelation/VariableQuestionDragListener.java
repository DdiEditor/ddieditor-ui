...
public class VariableQuestionDragListener implements DragSourceListener {
	private final Viewer viewer;
	String rcpPartId;

	public VariableQuestionDragListener(Viewer viewer, String rcpPartId) {
		super();
		this.viewer = viewer;
	...
	public void dragSetData(DragSourceEvent event) {
		...
		// set data on event
		int[] indices = null;
		if (VariableQuestionRelationTransfer.getInstance()
			...
			// set event data
			int count = 0;
			VariableQuestionRelationTransferVO[] result = new VariableQuestionRelationTransferVO[indices.length];
			for (Iterator iterator = selection.iterator(); iterator.hasNext(); count++) {
				result[count] = new VariableQuestionRelationTransferVO(rcpPartId,
						indices[count], (VariableQuestionRelation) iterator.next());
			}
			event.data = result;
		}
	}
