...
import org.ddialliance.ddi3.xml.xmlbeans.datacollection.MultipleQuestionItemNameDocument;
...
import org.ddialliance.ddi3.xml.xmlbeans.datacollection.impl.MultipleQuestionItemNameDocumentImpl;
...
public class MultipleQuestionItem extends Model {
	...
	private List<MaintainableLabelUpdateElement> maintainableUpdateNames = new ArrayList<MaintainableLabelUpdateElement>();
	private List<MaintainableLabelUpdateElement> maintainableUpdateConceptRefs = new ArrayList<MaintainableLabelUpdateElement>();
	private List<DynamicTextType> questionTexts = new ArrayList<DynamicTextType>(); // aka
																					// updates
	private List<NameType> names = new ArrayList<NameType>();
	...
	public MultipleQuestionItem(String id, String version, String parentId,
		...
		if (maintainableLabelQueryResult != null) {
			...
			XmlObject[] xmlObjects = maintainableLabelQueryResult
					.getSubElement("QuestionText");
			for (int i = 0; i < xmlObjects.length; i++) {
				maintainableUpdateTexts.add(new MaintainableLabelUpdateElement(
						((QuestionTextDocument) xmlObjects[i])
								.getQuestionText(), null /* NOP */));
				questionTexts
						.add((DynamicTextType) ((QuestionTextDocument) xmlObjects[i])
								.getQuestionText());
			}

			// get Name(s) from Query Result
			xmlObjects = maintainableLabelQueryResult
					.getSubElement("MultipleQuestionItemName");
			for (int i = 0; i < xmlObjects.length; i++) {
				System.out.println("Class name: " + xmlObjects[i].getClass());
				maintainableUpdateNames.add(new MaintainableLabelUpdateElement(
						((MultipleQuestionItemNameDocumentImpl) xmlObjects[i])
								.getMultipleQuestionItemName(), null /* NOP */));
				names.add(((MultipleQuestionItemNameDocumentImpl) xmlObjects[i])
						.getMultipleQuestionItemName());
			}

			// get ConceptReference(s) from Query Result
			xmlObjects = maintainableLabelQueryResult
					.getSubElement("ConceptReference");
			for (int i = 0; i < xmlObjects.length; i++) {
				// - store as StructuredString
				maintainableUpdateConceptRefs
						.add(new MaintainableLabelUpdateElement(
								((ConceptReferenceDocumentImpl) xmlObjects[i])
										.getConceptReference(), null /* NOP */));
				conceptReferences
						.add(((ConceptReferenceDocumentImpl) xmlObjects[i])
								.getConceptReference());
			}
			...
			if (xmlObjects.length > 0) {
				maintainableUpdateSubQuestionSeq = new MaintainableLabelUpdateElement(
						((SubQuestionSequenceDocumentImpl) xmlObjects[0])
								.getSubQuestionSequence(),
						null /* NOP */);
				subQuestionSeq = ((SubQuestionSequenceDocumentImpl) xmlObjects[0])
						.getSubQuestionSequence();
			}
		}
	...
	public MultipleQuestionItemDocument getDocument() throws DDIFtpException {
		...
		try {
			...
			// concept reference
			ReflectionUtil.invokeMethod(type, "setConceptReferenceArray",
					false, new Object[] { getConceptREeferenceTypeAsArray() });

			// sub-question sequence (optional)
			if (subQuestionSeq != null) {
				((MultipleQuestionItemTypeImpl) type)
						.setSubQuestionSequence(getSubQuestionSequence());
			}

			// question text
			ReflectionUtil.invokeMethod(type, "setQuestionTextArray", false,
					new Object[] { getQuestionTextAsArray() });

			// subquestions
			((MultipleQuestionItemTypeImpl) type)
					.setSubQuestions(QuestionGroupType.Factory.newInstance());
		} catch (Exception e) {
	...
	public void executeChange(Object value, Class<?> type) throws Exception {
		if (type.equals(NameType.class)) {
			// Set Name
			NameType name = getNameType();
			if (name == null) {
				name = MultipleQuestionItemNameDocument.Factory.newInstance()
						.addNewMultipleQuestionItemName();
				name.setStringValue((String) value);
				names.add(name);
			} else {
				names.get(0).setStringValue((String) value);
			}
		} else if (type.equals(ReferenceType.class)) {
			// Set Concept Reference
			ReferenceType ref = getConceptReferenceType();
			if (ref == null) {
				ref = ConceptReferenceDocument.Factory.newInstance()
						.addNewConceptReference();
				ref.addNewID().setStringValue(
						XmlBeansUtil.getXmlAttributeValue(
								((XmlObject) value).xmlText(), "id=\""));
				conceptReferences.add(ref);
				maintainableUpdateConceptRefs
						.add(new MaintainableLabelUpdateElement(ref,
								MaintainableLabelUpdateElement.NEW));
			} else {
				ModelAccessor.setReference(conceptReferences, ref,
			...
			if (!value.equals("")) {
				...
				if (maintainableUpdateSubQuestionSeq == null) {
					maintainableUpdateSubQuestionSeq = new MaintainableLabelUpdateElement(
							subSeqType, MaintainableLabelUpdateElement.NEW);
					subQuestionSeq = SubQuestionSequenceDocument.Factory
							.newInstance().addNewSubQuestionSequence();
				} else {
					maintainableUpdateSubQuestionSeq.setValue(subSeqType
							.xmlText());
				}
			...
			if (questionText == null) {
				if (value != null && !value.equals("")) {
					questionText = QuestionTextDocument.Factory.newInstance()
							.addNewQuestionText();
					...
					XmlBeansUtil.setTextOnMixedElement(lTextType.addNewText(),
							(String) value);
					questionTexts.add((DynamicTextType) questionText);
					maintainableUpdateTexts
							.add(new MaintainableLabelUpdateElement(
									questionText,
									MaintainableLabelUpdateElement.NEW));
				}
