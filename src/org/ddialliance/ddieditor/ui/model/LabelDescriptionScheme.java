...
public abstract class LabelDescriptionScheme extends Model implements IModel,
		...
		if (maintainableLabelQueryResult != null) {
			...
			for (int i = 0; i < xmlObjects.length; i++) {
				...
				maintainableUpdateLabels
						.add(new MaintainableLabelUpdateElement(
								((LabelDocument) xmlObjects[i]).getLabel(),
								null /* NOP */));
			...
			for (int i = 0; i < xmlObjects.length; i++) {
				...
				maintainableUpdateDescriptions
						.add(new MaintainableLabelUpdateElement(
								((DescriptionDocument) xmlObjects[i])
										.getDescription(), null /* NOP */));
				descrs.add(((DescriptionDocument) xmlObjects[i])
						.getDescription());
			}
		}
	...
	public static MaintainableLabelQueryResult createLabelDescriptionScheme(
		...
		maintainableLabelQueryResult.setId(XmlBeansUtil.getXmlAttributeValue(
				xmlObject.xmlText(), "id=\""));
	...
	private List<MaintainableLabelUpdateElement> getDirtyUpdateElements(
		...
		for (MaintainableLabelUpdateElement maintainableLabelUpdateElement : maintainableLabelUpdateElements) {
			if (maintainableLabelUpdateElement.getXmlObject() instanceof LabelType) {
				LabelDocument labelDocument = LabelDocument.Factory
						.newInstance();
				labelDocument
						.setLabel((LabelType) maintainableLabelUpdateElement
								.getXmlObject());
				maintainableLabelUpdateElement.setValue(labelDocument
						.xmlText(xmlOptions));
				...
				DescriptionDocument descriptionDocument = DescriptionDocument.Factory
						.newInstance();
				descriptionDocument
						.setDescription((StructuredStringType) maintainableLabelUpdateElement
								.getXmlObject());
				maintainableLabelUpdateElement.setValue(descriptionDocument
						.xmlText(xmlOptions));
	 ...
	 * @param updates
	 * @param txt
	 *            text input of element
	 * @throws DDIFtpException
	 */
	private void syncMaintainableLabelUpdateElementsWithUpdates(
			String txt,
			List<MaintainableLabelUpdateElement> maintainableLabelUpdateElements,
		...
		for (int i = 0; i < maintainableLabelUpdateElements.size(); i++) {
			...
			for (int j = 0; j < updates.size(); j++) {
				try {
					if (updates.get(j) == maintainableLabelUpdateElements
							.get(i).getXmlObject()) {
						found = true;
						break;
					}
			...
			if (found && txt.length() > 0) {
				...
				if (updates.get(i) instanceof LabelType) {
					...
					maintainableUpdateDescriptions.get(i).setValue(
	 ...
	 * @throws DDIFtpException
	 */
	private void syncUpdatesWithMaintainableLabelUpdateElements(List updates,
		...
		for (int i = 0; i < updates.size(); i++) {
			...
			if (!found) {
				if (updates.get(i) instanceof LabelType) {
					...
					throw new DDIFtpException("Unsupported object name: "
							+ updates.getClass().getName());
				}
			}
		}
	...
	private void syncUpdatesWithMaintainableLabelUpdateElement(
			XmlObject update,
			List<MaintainableLabelUpdateElement> maintainableLabelUpdateElements)
			throws DDIFtpException {
		for (int j = 0; j < maintainableLabelUpdateElements.size(); j++) {
			try {
				if (update == maintainableLabelUpdateElements.get(j)
						.getXmlObject()) {
					maintainableLabelUpdateElements.get(j).setValue(
							update.xmlText(xmlOptions));
					return;
				}
			} catch (Exception e) {
				throw new DDIFtpException(e);
		...
		throw new DDIFtpException(
				Translator
						.trans("Editor.mess.MaintainableLabelUpdateElementNotFound"));
	}
	...
	public LabelType setDisplayLabel(String text) throws DDIFtpException {
		...
		syncMaintainableLabelUpdateElementsWithUpdates(text,
				maintainableUpdateLabels, labels);
		syncUpdatesWithMaintainableLabelUpdateElements(labels,
				maintainableUpdateLabels);
		...
		if (labels.size() > 0) {
			...
			if (label != null) {
				...
				if (text.length() > 0) {
					...
					syncUpdatesWithMaintainableLabelUpdateElement(label,
							maintainableUpdateLabels);
					return label;
				} else {
					// remove label:
		...
		if (label == null && text != null && !text.equals("")) {
			...
			maintainableUpdateLabels.add(new MaintainableLabelUpdateElement(
					label, MaintainableLabelUpdateElement.NEW));
		}
	...
	public StructuredStringType setDisplayDescr(String text)
		...
		syncUpdatesWithMaintainableLabelUpdateElements(descrs,
				maintainableUpdateDescriptions);
		...
		if (descrs.size() > 0) {
			...
			if (descr != null) {
				if (text.length() > 0) {
					...
					syncUpdatesWithMaintainableLabelUpdateElement(descr,
							maintainableUpdateDescriptions);
					return descr;
				}
			}
